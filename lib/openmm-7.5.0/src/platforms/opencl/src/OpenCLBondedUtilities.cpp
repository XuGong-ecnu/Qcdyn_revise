...
void OpenCLBondedUtilities::addInteraction(const vector<vector<int> >& atoms, const string& source, int group) {
    if (atoms.size() > 0) {
        ...
        int topologyGroup = group / 32;
        // Get the internal force group in its Topology.
        int internalForceGroup = group % 32;
        int allGroup = 1 << internalForceGroup;
        // A topologyGroup already exists.
        if (allGroups.size() > topologyGroup)
            allGroups[topologyGroup] |= allGroup;
        // A new topologyGroup. 
        else {
            allGroups.resize((topologyGroup+1), 0);
            allGroups.back() = allGroup;
        }
        int width = 1;
...
void OpenCLBondedUtilities::initialize(const System& system) {
    ...
    for (auto& set : forceSets) {
        ...
        s<<"__kernel void computeBondedForces(__global "<<bufferType<<"* restrict forceBuffers, __global mixed* restrict energyBuffer, __global const real4* restrict posq, __global const int* groups, real4 periodicBoxSize, real4 invPeriodicBoxSize, real4 periodicBoxVecX, real4 periodicBoxVecY, real4 periodicBoxVecZ";
...
string OpenCLBondedUtilities::createForceSource(int forceIndex, int numBonds, int numAtoms, int group, const string& computeForce) {
    ...
    stringstream s;
    int topologyGroup = group / 32 ;
    // if ((topologyGroup < groups.size()) && (groups[topologyGroup]&(1<<(group-32*topologyGroup))) != 0))
    // Note that here, the size of groups is groups[0].
    s<<"if (("<<topologyGroup<<" < "<<"groups[0]) && ((groups["<<(topologyGroup+1)<<"]&"<<(1<<(group-32*topologyGroup))<<") != 0))\n";
...
void OpenCLBondedUtilities::computeInteractions(const std::vector<int>& groups) {
    int min = std::min<int>(groups.size(), allGroups.size());
    for (int i = 0; i < min; i++) {
        if ((groups[i]&allGroups[i]) != 0) break;
        if (i == (min-1)) return;
    }
    ...
    for (int i = 0; i < (int) kernels.size(); i++) {
        cl::Kernel& kernel = kernels[i];
        // Copy groups and insert the size of groups to first position.
        std::vector<int> copyGroups = groups;
        copyGroups.insert(copyGroups.begin(), copyGroups.size());    
        // Create a OpenCLArray object to store copyGroups (vector<int>).
        OpenCLArray groupsBuffer(context, copyGroups.size(), sizeof(int), "groupsBuffer");
        // Copy the values in copyGroups (vector<int>) to the device memory.
        groupsBuffer.upload(copyGroups);        
        kernel.setArg<cl::Buffer>(3, groupsBuffer.getDeviceBuffer());
