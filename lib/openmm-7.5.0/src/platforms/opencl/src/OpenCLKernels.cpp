...
void OpenCLCalcForcesAndEnergyKernel::beginComputation(ContextImpl& context, bool includeForces, bool includeEnergy, const std::vector<int>& groups) {
...
double OpenCLCalcForcesAndEnergyKernel::finishComputation(ContextImpl& context, bool includeForces, bool includeEnergy, const std::vector<int>& groups, bool& valid) {
...
// get/set current step. (Added by zhubin, Nov. 17, 2021)
int OpenCLUpdateStateDataKernel::getStep(const ContextImpl& context) const {
    return cl.getStepCount();
}

void OpenCLUpdateStateDataKernel::setStep(ContextImpl& context, int step) {
    vector<OpenCLContext*>& contexts = cl.getPlatformData().contexts;
    for (auto ctx : contexts)
        ctx->setStepCount(step);
}
...
// Note: in the integrator, the longForceBuffer will be used.
// This is simillar as UpdateStateDataKernel::setForces()
void OpenCLUpdateStateDataKernel::setForces(ContextImpl& context, const vector<Vec3>& forces) {
    const vector<cl_int>& order = cl.getAtomIndex();
    int numParticles = context.getSystem().getNumParticles();
    int paddedNumParticles = cl.getPaddedNumAtoms();
    cl_long* longForceBuffer = (cl_long*) cl.getPinnedBuffer();
    cl.getLongForceBuffer().download(longForceBuffer);
    double inverseScale = (double) 0x100000000LL;
    for (int i = 0; i < numParticles; ++i) {
        longForceBuffer[i] = inverseScale * forces[order[i]][0];
        longForceBuffer[i+paddedNumParticles] = inverseScale * forces[order[i]][1];
        longForceBuffer[i+paddedNumParticles*2] = inverseScale * forces[order[i]][2];
    }
    cl.getLongForceBuffer().upload(longForceBuffer);
}
...
public:
    ...
    void computeForceAndEnergy(bool includeForces, bool includeEnergy, const std::vector<int>& groups) {
...
public:
    ...
    double computeForceAndEnergy(bool includeForces, bool includeEnergy, const std::vector<int>& groups) {
...
double OpenCLCalcCustomCVForceKernel::execute(ContextImpl& context, ContextImpl& innerContext, bool includeForces, bool includeEnergy) {
    ...
    for (int i = 0; i < numCVs; i++) {
        if (i == 32) break;
        cvValues.push_back(innerContext.calcForcesAndEnergy(true, true, std::vector<int>(1, (1<<i))));
