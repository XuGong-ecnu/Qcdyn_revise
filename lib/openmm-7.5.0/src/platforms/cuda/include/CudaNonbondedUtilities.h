...
public:
     ...
     */
    void prepareInteractions(const std::vector<int>& forceGroups);
    /**
     ...
     */
    void computeInteractions(const std::vector<int>& forceGroups, bool includeForces, bool includeEnergy);
    /**
     ...
     */
    CUfunction createInteractionKernel(const std::string& source, std::vector<ParameterInfo>& params, std::vector<ParameterInfo>& arguments, bool useExclusions, bool isSymmetric, const std::vector<int>& groups, bool includeForces, bool includeEnergy);
    /**
     ...
     */
    void createKernelsForGroups(const std::vector<int>& groups);
    /**
...
private:
    ...
    CudaContext& context;
    std::map<std::vector<int>, KernelSet> groupKernels;
    ...
    int startTileIndex, startBlockIndex, numBlocks, maxTiles, maxSinglePairs, maxExclusions, numForceThreadBlocks, forceThreadBlockSize, numAtoms;
    std::vector<int> groupFlags;
    long long numTiles;
