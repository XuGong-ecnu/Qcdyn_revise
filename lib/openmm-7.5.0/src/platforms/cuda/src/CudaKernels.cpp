...
void CudaCalcForcesAndEnergyKernel::beginComputation(ContextImpl& context, bool includeForces, bool includeEnergy, const std::vector<int>& groups) {
...
double CudaCalcForcesAndEnergyKernel::finishComputation(ContextImpl& context, bool includeForces, bool includeEnergy, const std::vector<int>& groups, bool& valid) {
    cu.setAsCurrent();
...
int CudaUpdateStateDataKernel::getStep(const ContextImpl& context) const {
    return cu.getStepCount();
}

void CudaUpdateStateDataKernel::setStep(ContextImpl& context, int step) {
    vector<CudaContext*>& contexts = cu.getPlatformData().contexts;
    for (auto ctx : contexts)
...
void CudaUpdateStateDataKernel::setForces(ContextImpl& context, const vector<Vec3>& forces) {
    cu.setAsCurrent();
    const vector<int>& order = cu.getAtomIndex();
    int numParticles = context.getSystem().getNumParticles();
    int paddedNumParticles = cu.getPaddedNumAtoms();
    ...
    for (int i = 0; i < numParticles; ++i) {
        force[i] = inverseScale * forces[order[i]][0];
        force[i+paddedNumParticles] = inverseScale * forces[order[i]][1];
        force[i+paddedNumParticles*2] = inverseScale * forces[order[i]][2];
    }
