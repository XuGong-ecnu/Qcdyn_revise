...
void CudaBondedUtilities::initialize(const System& system) {
    ...
    for (int i = 0; i < (int) prefixCode.size(); i++)
        s<<prefixCode[i];
    s<<"extern \"C\" __global__ void computeBondedForces(unsigned long long* __restrict__ forceBuffer, mixed* __restrict__ energyBuffer, const real4* __restrict__ posq, const int* __restrict__ groups, real4 periodicBoxSize, real4 invPeriodicBoxSize, real4 periodicBoxVecX, real4 periodicBoxVecY, real4 periodicBoxVecZ";
...
void CudaBondedUtilities::computeInteractions(const std::vector<int>& groups) {
    ...
    // Create a CudaArray object to store copyGroups (vector<int>).
    CudaArray groupsBuffer(context, copyGroups.size(), sizeof(int), "groupsBuffer");
    // Copy the values in copyGroups (vector<int>) to the device memory.
    groupsBuffer.upload(copyGroups);
    kernelArgs[3] = &groupsBuffer.getDevicePointer();
