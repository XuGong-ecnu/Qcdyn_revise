...
CudaNonbondedUtilities::CudaNonbondedUtilities(CudaContext& context) : context(context), useCutoff(false), usePeriodic(false), anyExclusions(false), usePadding(true),
        blockSorter(NULL), pinnedCountBuffer(NULL), forceRebuildNeighborList(true), lastCutoff(0.0), canUsePairList(true) {
...
void CudaNonbondedUtilities::prepareInteractions(const std::vector<int>& forceGroups) {
...
void CudaNonbondedUtilities::computeInteractions(const std::vector<int>& forceGroups, bool includeForces, bool includeEnergy) {
    //if ((forceGroups&groupFlags) == 0)
    //    return;    
    // Here, check if the forcegroups contains the force groups in groupFlags
    for (int i = 0; i < groupFlags.size(); i++) {
...
void CudaNonbondedUtilities::createKernelsForGroups(const std::vector<int>& groups) {
    KernelSet kernels;
    double cutoff = 0.0;
    string source;
    //for (int i = 0; i < 32; i++) {
...
CUfunction CudaNonbondedUtilities::createInteractionKernel(const string& source, vector<ParameterInfo>& params, vector<ParameterInfo>& arguments, bool useExclusions, bool isSymmetric, const std::vector<int>& groups, bool includeForces, bool includeEnergy) {
    map<string, string> replacements;
