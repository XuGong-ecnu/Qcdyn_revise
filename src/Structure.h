...
/**
 * This class represents structure of a system, including atom information,
 * positions, velocities, forces, and box vectors, which can be used to load
 * structure file and store structure data and save structure/trajectory file.
...
public:
    ...
    * as positions, are empty. You can initialize the data using loadStructure()
    ...
    * This will override the existing structure data (if any).
    *
    ...
    */
    ...
    /**
    * Save data to structure file.
    * This will override the existing file with same name (if any).
    *
    * Currently, the supported file is Gromacs gro file.
    *
    * @param file  the file name of structure file to save, the extension name
    *              of it indicates the format.
    ...
    */
     ...
     */
    void getBoxVectors(OpenMM::Vec3& a, OpenMM::Vec3& b, OpenMM::Vec3& c) const;
    /**
     ...
     */
    std::vector<OpenMM::Vec3>& getVelocities() {
        return velocities;
    }
    /**
     ...
     */
    auto getBoxVectors() -> OpenMM::Vec3(&)[3] {
        return boxVectors;
    }
    /**
     ...
     * y direction, and c must point "mostly" in the z direction. (The requirment
     * of triclinic is the same as in Gromacs (see its manual)).
     *
     * See the OpenMM documentation for details:
     * http://docs.openmm.org/latest/userguide/theory/05_other_features.html
     *
     * @param a      the vector defining the first edge of the periodic box
     * @param b      the vector defining the second edge of the periodic box
     * @param c      the vector defining the third edge of the periodic box
     */
    void setBoxVectors(const OpenMM::Vec3& a, const OpenMM::Vec3& b, const OpenMM::Vec3& c);
...
private:
    ...
    */
     ...
     * @param c    the vector defining the third edge of the periodic box
     */
    void checkBoxVectors(const OpenMM::Vec3& a, const OpenMM::Vec3& b, const OpenMM::Vec3& c) const;
...
private:
    ...
    // The coordinate (in nm) of each atom in OpenMM::Vec3 format.
    std::vector<OpenMM::Vec3> positions;
    // The velocity in (nm/ps) of each atom in OpenMM::Vec3 format.
    std::vector<OpenMM::Vec3> velocities;
    // The forces in (kj/mol/nm) of each atom in OpenMM::Vec3 format.
